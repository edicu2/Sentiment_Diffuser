...
public class ProgressCircle1 extends View {
    ...
    public ProgressCircle1(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initAttributes(Context context, AttributeSet attrs) {
        TypedArray attributes = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SeekCircle1, 0, 0);
        try {
            // Read and clamp min
            int min = attributes.getInteger(R.styleable.SeekCircle1_min, 0);
            mMinProgress = Math.max(min, 0);

            // Read and clamp max
            int max = attributes.getInteger(R.styleable.SeekCircle1_max, 100);
            mMaxProgress = Math.max(max, 1);

            // Read and clamp progress
            int progress = attributes.getInteger(R.styleable.SeekCircle1_progress, 0);
            mProgress = Math.max(Math.min(progress, mMaxProgress), 0);

            mColor1 = Color.parseColor(getResources().getString(0+R.color.neutral));
            mColor2 = Color.parseColor(getResources().getString(0+R.color.neutral));
            mInactiveColor = Color.parseColor(getResources().getString(0+R.color.colorPrimaryfont));

            mRingBias = attributes.getFloat(R.styleable.SeekCircle1_ringBias, 0.15f);
            mSectionRatio = attributes.getFloat(R.styleable.SeekCircle1_sectionRatio, 5.0f);
        } finally {
            attributes.recycle();
        }
    }
