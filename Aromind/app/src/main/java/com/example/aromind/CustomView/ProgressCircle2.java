...
public class ProgressCircle2 extends View {
    ...
    private int mColor1 = Color.parseColor("#000000");
    private int mColor2 = Color.parseColor("#000000");
    private int mInactiveColor = Color.parseColor("#000000");
    ...
    public ProgressCircle2(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        initAttributes(context, attrs);
    }

    public ProgressCircle2(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void initAttributes(Context context, AttributeSet attrs) {
        TypedArray attributes = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SeekCircle2, 0, 0);
        try {
            // Read and clamp min
            int min = attributes.getInteger(R.styleable.SeekCircle2_min, 0);
            mMinProgress = Math.max(min, 0);

            // Read and clamp max
            int max = attributes.getInteger(R.styleable.SeekCircle2_max, 100);
            mMaxProgress = Math.max(max, 1);

            // Read and clamp progress
            int progress = attributes.getInteger(R.styleable.SeekCircle2_progress, 0);
            mProgress = Math.max(Math.min(progress, mMaxProgress), 0);

            mColor1 = Color.parseColor(getResources().getString(0+R.color.negative));
            mColor2 = Color.parseColor(getResources().getString(0+R.color.negative));
            mInactiveColor = Color.parseColor(getResources().getString(0+R.color.colorPrimaryfont));

            mRingBias = attributes.getFloat(R.styleable.SeekCircle2_ringBias, 0.15f);
            mSectionRatio = attributes.getFloat(R.styleable.SeekCircle2_sectionRatio, 5.0f);
        } finally {
            attributes.recycle();
        }
    }
